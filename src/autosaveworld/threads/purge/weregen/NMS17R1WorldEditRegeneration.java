...
public class NMS17R1WorldEditRegeneration implements WorldEditRegenrationInterface {
	...
	public void regenerateRegion(World world, Vector minpoint, Vector maxpoint, RegenOptions options) {
		...
		for (Vector2D chunk : region.getChunks()) {
			net.minecraft.server.v1_7_R1.WorldServer nmsWorld = ((CraftWorld)world).getHandle();
			...
			//generate nms chunk
			net.minecraft.server.v1_7_R1.Chunk nmschunk = null;
			...
			if (fullyinside) {//if chunk is fully inside the region than we just update it
				...
				//save old chunk
				net.minecraft.server.v1_7_R1.Chunk oldnsmchunk = nmsWorld.chunkProviderServer.chunks.get(LongHash.toLong(cx, cz));
