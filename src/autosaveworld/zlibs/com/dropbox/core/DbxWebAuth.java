...
/**
 * Does the OAuth 2 "authorization code" flow. (This SDK does not support the
 * "token" flow.)
 * 
 * <p>
 * Eventually yields an access token, which can be used with {@link DbxClient}
 * to make Dropbox API calls. You typically only need to do this for a user when
 * they first use your application. Once you have an access token for that user,
 ...
 * 
 * <p>
 * Servlet Example, part 2 (handler for "http://my-server.com/dropbox-auth";
 * doesn't include error-handling code)
 * </p>
 * 
 * <pre>
...
public class DbxWebAuth {
	...
	private final StringStorage csrfTokenStore = new StringStorage();
	...
	/**
	 * @param appInfo
	 *            Your application's Dropbox API information (the app key and
	 *            secret).
	 */
	public DbxWebAuth(DbxRequestConfig requestConfig, DbxAppInfo appInfo, String redirectUri, String csrfToken) {
	...
	/**
	 * Start authorization. Returns a "authorization URL" on the Dropbox website
	 * that gives the lets the user grant your app access to their Dropbox
	 * account.
	 * 
	 * <p>
	 * If they choose to grant access, they will be shown an
	 * "authorization code", which they need to copy/paste back into your app,
	 * at which point you can call {@link #finish} to get an access token.
	 * </p>
	 */
	 ...
	 * redirected them back to you (using the {@code redirectUri} you passed in
	 * to {@link #start}.
	 * 
	 * @param queryParams
	 *            The query parameters on the GET request to your
	 *            {@code redirectUri}.
	 */
	public DbxAuthFinish finish(Map<String, String[]> queryParams)
		...
		if (code == null && error == null)
			throw new BadRequestException(
					"missing both 'code' and 'error'; one must be present");
		if (code != null && error != null)
			throw new BadRequestException(
					"both 'code' and 'error' are set; only one must be present");
		if (code != null && errorDescription != null)
			throw new BadRequestException(
					"both 'code' and 'error_description' are set");
		...
		if (csrfTokenFromSession.length() <= 20)
			throw new AssertionError("CSRF token too short: " + jq(csrfTokenFromSession));
		...
		if (error != null) {
			if (error.equals("access_denied")) {
				...
				if (errorDescription == null) {
					...
					exceptionMessage = "Additional description from Dropbox: "
							+ errorDescription;
				}
		...
		DbxAuthFinish finish = DbxWebAuthHelper.finish(this.appInfo,
				this.requestConfig, code, this.redirectUri);
		return new DbxAuthFinish(finish.accessToken, finish.userId,
				givenUrlState);
	}
	...
	/**
	 * The base class for authorization redirect errors. You should catch each
	...
	/**
	 * Thrown when the parameters passed to your redirect URI are not
	 * well-formed.
	 * 
	 * <p>
	 * IMPORTANT: The exception's message must not be shown the the user, but
	 * may be logged.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * Thrown if all the parameters to your redirect URI are well-formed, but
	 * there's no CSRF token in the session. This probably means that the user's
	 * session expired and they must restart the OAuth 2 process.
	 * 
	 * <p>
	 * IMPORTANT: The exception's message must not be shown the the user, but
	 * may be logged.
	...
	public static final class BadStateException extends Exception {
		...
		public BadStateException() {
			super(
					"Not expecting Dropbox auth redirect (session doesn't have CSRF token)");
		}
	}

	/**
	 * Thrown if the given 'state' parameter doesn't contain the expected CSRF
	 * token. This request should be blocked to prevent CSRF attacks.
	 * 
	 * <p>
	 * IMPORTANT: The exception's message must not be shown the the user, but
	 * may be logged.
	...
	/**
	 * Thrown when Dropbox tells us that the user chose not to grant your app
	 * access to their Dropbox account (i.e. the user clicked the "Deny"
	 * button).
	 * 
	 * <p>
	 * IMPORTANT: The exception's message must not be shown the the user, but
	...
	/**
	 * Thrown when Dropbox tells us that some other error occurred in the
	 * authorization process.
	 * 
	 * <p>
	 * IMPORTANT: The exception's message should not be shown the the user, but
	 * should be logged.
	 * </p>
	 */
