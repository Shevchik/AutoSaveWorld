...
public class DbxRequestUtil {
	...
	public static String buildUrlWithParams(String userLocale, String host,
	...
	public static String buildUri(String host, String path) {
		try {
			return new URI("https", host, "/" + path, null).toURL()
					.toExternalForm();
	...
	private static String encodeUrlParams(String userLocale, String[] params) {
		...
		if (params != null) {
			if (params.length % 2 != 0) {
				throw new IllegalArgumentException("'params.length' is "
						+ params.length + "; expecting a multiple of two");
			}
	...
	private static ArrayList<HttpRequestor.Header> addAuthHeader(
			ArrayList<HttpRequestor.Header> headers, String accessToken) {
		if (headers == null)
	...
	public static ArrayList<HttpRequestor.Header> addUserAgentHeader(
	...
	public static HttpRequestor.Header buildUserAgentHeader(
			DbxRequestConfig requestConfig) {
		return new HttpRequestor.Header("User-Agent",
				requestConfig.clientIdentifier + " Dropbox-Java-SDK/"
	 ...
	 */
	public static HttpRequestor.Response startGet(
			DbxRequestConfig requestConfig, String accessToken, String host,
			String path, String[] params,
			ArrayList<HttpRequestor.Header> headers)
		...
		String url = buildUrlWithParams(requestConfig.userLocale, host, path,
				params);
		try {
	 ...
	 */
	public static HttpRequestor.Uploader startPut(
			DbxRequestConfig requestConfig, String accessToken, String host,
			String path, String[] params,
			ArrayList<HttpRequestor.Header> headers)
			throws DbxException.NetworkIO {
	 ...
	 */
	public static HttpRequestor.Response startPostNoAuth(
			DbxRequestConfig requestConfig, String host, String path,
			String[] params, ArrayList<HttpRequestor.Header> headers)
			throws DbxException.NetworkIO {
		String uri = buildUri(host, path);
		byte[] encodedParams = StringUtil.stringToUtf8(encodeUrlParams(
				requestConfig.userLocale, params));

		headers = addUserAgentHeader(headers, requestConfig);
		headers.add(new HttpRequestor.Header("Content-Type",
				"application/x-www-form-urlencoded; charset=utf-8"));
		headers.add(new HttpRequestor.Header("Content-Length", Integer
				.toString(encodedParams.length)));

		try {
			HttpRequestor.Uploader uploader = requestConfig.httpRequestor
					.startPost(uri, headers);
			try {
	...
	public static abstract class ResponseHandler<T> {
		public abstract T handle(HttpRequestor.Response response)
				throws DbxException;
	}

	public static Map<String, String> parseAsQueryString(InputStream in) throws DbxException {
	...
	public static <T> T doGet(DbxRequestConfig requestConfig,
			String accessToken, String host, String path, String[] params,
			ArrayList<HttpRequestor.Header> headers, ResponseHandler<T> handler)
			throws DbxException {
	...
	public static <T> T finishResponse(HttpRequestor.Response response,
			ResponseHandler<T> handler) throws DbxException {
		try {
			return handler.handle(response);
		} finally {
	...
	public static String getFirstHeader(HttpRequestor.Response response,
			String name) throws DbxException {
	...
	public static String getFirstHeaderMaybe(HttpRequestor.Response response,
			String name) throws DbxException {
		List<String> values = response.headers.get(name);
		if (values == null)
			return null;
	...
	public static <T, E extends Throwable> T runAndRetry(int maxTries,
			RequestMaker<T, E> requestMaker) throws DbxException, E {
		int numTries = 0;
		while (true) {
			try {
				numTries++;
	...
	private static boolean isRetriableException(DbxException ex) {
		return ex instanceof DbxException.RetryLater
				|| ex instanceof DbxException.ServerError;
	}
}
