...
/**
 * The base exception thrown by Dropbox API calls. Normally, you'll need to do something specific for {@link InvalidAccessToken} and possibly for {@link RetryLater}. The rest you should probably just log and investigate manually.
 */
public class DbxException extends Exception {
	 ...
	 * <li>Sometimes you're actually making an invalid request, but for some reason the server's validation logic isn't catching the problem and it's causing the server to error-out somewhere down the line. You may be able to fix your request and get things working. This is still a bug on Dropbox's end, so you should still report it on the Dropbox API forums.</li>
	 * <li>Sometimes the error is intermittent. It may only happen once every few requests, in which case you can temporarily work around the issue by retrying the request. Again, this still counts as a bug so report it on the forums.</li>
	 * <li>Sometimes the error is temporary. For example, Dropbox might be experiencing a temporary network or hardware issue. If you wait a few minutes, the error might stop happening. You might want to check the forums to see if other people are seeing the same thing.</li>
	 * </ul>
	 */
	 ...
	 *
	 * <p>
	 * A more typical situation is that your access token <em>was</em> valid, but the user has since "unlinked" your application via the Dropbox website (<a href="http://www.dropbox.com/account#applications">http://www. dropbox.com/account#applications</a>). When a user unlinks your application, your access tokens for that user become invalid. You can re-run the authorization process to obtain a new access token.
	 * </p>
	 */
