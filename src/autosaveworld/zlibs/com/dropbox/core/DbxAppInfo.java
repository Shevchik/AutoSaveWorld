 ...
 */
public class DbxAppInfo implements java.io.Serializable {
	public static final long serialVersionUID = 0;

	/**
	 * Your Dropbox <em>app key</em> (OAuth calls this the <em>consumer key</em>
	 * ). You can create an app key and secret on the <a
	 * href="http://dropbox.com/developers/apps">Dropbox developer website</a>.
	 */
	public final String key;

	/**
	 * Your Dropbox <em>app secret</em> (OAuth calls this the
	 * <em>consumer secret</em>). You can create an app key and secret on the <a
	 * href="http://dropbox.com/developers/apps">Dropbox developer website</a>.
	 * 
	 * <p>
	 * Make sure that this is kept a secret. Someone with your app secret can
	 * impesonate your application. People sometimes ask for help on the Dropbox
	 * API forums and copy/paste their code, which sometimes includes their app
	 * secret. Do not do that.
	 * </p>
	 */
	public final String secret;

	/**
	 * This is almost always {@link DbxHost#Default}. This is only set
	 ...
	 * 
	 * <p>
	 * NOTE: This function only performs some cursory checks on the format of
	 * the key. Even if it * returns {@code null} (which means "no problem"), it
	 * doesn't mean that what you passed in is an actual valid Dropbox API app
	 * key.
	 * </p>
	 */
	...
	/**
	 * If they secret's format looks correct, return {@code null}. Otherwise
	 * return a string that describes what the problem is.
	 * 
	 * <p>
	 * NOTE: This function only performs some cursory checks on the format. Even
	 * if it returns {@code null} (which means "no problem"), it doesn't mean
	...
	public static final JsonReader<DbxAppInfo> Reader = new JsonReader<DbxAppInfo>() {
		@Override
		public final DbxAppInfo read(JsonParser parser) throws IOException,
				JsonReadException {
			...
			while (parser.getCurrentToken() == JsonToken.FIELD_NAME) {
				...
				try {
					if (fieldName.equals("key")) {
						...
						secret = SecretReader.readField(parser, fieldName,
								secret);
	...
	public static final JsonReader<String> KeyReader = new JsonReader<String>() {
		...
		public String read(JsonParser parser) throws IOException,
			...
			try {
				...
				if (error != null) {
					throw new JsonReadException("bad format for app key: "
							+ error, parser.getTokenLocation());
				}
				parser.nextToken();
				return v;
	...
	public static final JsonReader<String> SecretReader = new JsonReader<String>() {
		...
		public String read(JsonParser parser) throws IOException,
			...
			try {
				...
				String error = getKeyFormatError(v);
				if (error != null) {
					throw new JsonReadException("bad format for app secret: "
							+ error, parser.getTokenLocation());
				}
				parser.nextToken();
				return v;
