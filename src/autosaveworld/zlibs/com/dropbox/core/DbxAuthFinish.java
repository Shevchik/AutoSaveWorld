...
/**
 * When you successfully complete the authorization process, the Dropbox server
 * returns this information to you.
 */
public final class DbxAuthFinish implements java.io.Serializable {
	public static final long serialVersionUID = 0;

	/**
	 * An <em>access token</em> that can be used to make Dropbox API calls. Pass
	 * this in to the {@link DbxClient} constructor.
	 */
	...
	/**
	 * The state data you passed in to {@link DbxWebAuth#start}. If you didn't
	 * pass anything in, or you used {@link DbxWebAuthNoRedirect}, this will be
	 * {@code null}.
	 */
	...
	public static final JsonReader<DbxAuthFinish> Reader = new JsonReader<DbxAuthFinish>() {
		public DbxAuthFinish read(JsonParser parser) throws IOException,
				JsonReadException {
			...
			while (parser.getCurrentToken() == JsonToken.FIELD_NAME) {
				...
				try {
					if (fieldName.equals("token_type")) {
						tokenType = BearerTokenTypeReader.readField(parser,
								fieldName, tokenType);
					} else if (fieldName.equals("access_token")) {
						accessToken = AccessTokenReader.readField(parser,
								fieldName, accessToken);
					} else if (fieldName.equals("uid")) {
						userId = JsonReader.StringReader.readField(parser,
								fieldName, userId);
					} else {
		...
		public String read(JsonParser parser) throws IOException,
			...
			try {
				...
				if (!v.equals("Bearer") && !v.equals("bearer")) {
					throw new JsonReadException("expecting \"Bearer\": got "
							+ jq(v), parser.getTokenLocation());
				}
				parser.nextToken();
				return v;
