...
import autosaveworld.zlibs.com.dropbox.core.util.*;
...
public final class DbxClient {
	...
	private <T> Maybe<T> getMetadataWithChildrenIfChangedBase(String path, String previousFolderHash, final JsonReader<T> reader) throws DbxException {
		...
		return doGet(host, apiPath, params, null,
			new DbxRequestUtil.ResponseHandler<Maybe<T>>() {
				@Override
				public Maybe<T> handle(HttpRequestor.Response response)
						throws DbxException {
					if (response.statusCode == 404)
					...
					if (response.statusCode != 200)
						throw DbxRequestUtil.unexpectedStatus(response);
					return Maybe.Just(DbxRequestUtil.readJsonFromResponse(
	 ...
	 * 
	 * @param rev
	 *            The {@link DbxEntry.File#rev rev} of the file to retrieve, or
	 *            {@code null} if you want the latest revision of the file.
	 * 
	 * @throws IOException
	...
	/**
	 * Retrieve a file's content and content metadata. Returns a
	 * {@link Downloader} which is just an {@link InputStream} (can be used to
	 * read the file contents) and a {@link DbxEntry.File} (the file's
	 * metadata).
	 * 
	 * <p>
	 * You need to close the {@link Downloader} yourself. Use a {@code try}/
	 * {@code finally} to make sure you close it in all cases.
	 * </p>
	 * 
	 * <pre>
	 * DbxClient dbxClient = ...
	 ...
	 *            (see {@link DbxPath}).
	 */
	public Downloader startGetFile(final String path, String revision) throws DbxException {
	 ...
	 */
	private Downloader startGetSomething(final String apiPath, String[] params) throws DbxException {
		String host = this.host.content;

		boolean passedOwnershipOfStream = false;
		HttpRequestor.Response response = DbxRequestUtil.startGet(
				requestConfig, accessToken, host, apiPath, params, null);
		try {
			if (response.statusCode == 404)
				return null;
			if (response.statusCode != 200)
			...
			String metadataString = DbxRequestUtil.getFirstHeader(response,
					"x-dropbox-metadata");
			DbxEntry metadata;
			try {
			...
			if (metadata instanceof DbxEntry.Folder) {
				throw new DbxException.BadResponse(
						"downloaded file, but server returned metadata entry for a folder");
			}
			...
			// If we haven't passed ownership the stream to the caller, then
			// close it.
			if (!passedOwnershipOfStream) {
				try {
					response.body.close();
				} catch (IOException ex) {
					// Ignore, since we don't actually care about the data in
					// this method.
					// We only care about IOExceptions when actually reading
					// from the stream.
				}
			}
		}
	...
	/**
	 * A pairing of the file content's metadata and an {@link InputStream} to
	 * read the file content. Make sure you always close the {@code InputStream}
	 * .
	 */
