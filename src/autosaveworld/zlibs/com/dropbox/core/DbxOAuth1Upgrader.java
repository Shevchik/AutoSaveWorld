 ...
 * {@link #createOAuth2AccessToken} to get an OAuth 2 access token. If that
 * succeeds, call {@link #disableOAuth1AccessToken} to disable the OAuth 1
 * access token.
 */
public final class DbxOAuth1Upgrader {
	...
	/**
	 * Given an existing active OAuth 1 access token, make a Dropbox API call to
	 * get a new OAuth 2 access token that represents the same user and app.
	 */
	public String createOAuth2AccessToken(DbxOAuth1AccessToken token)
			throws DbxException {
		if (token == null)
			throw new IllegalArgumentException("'token' can't be null");
		return DbxRequestUtil.doPostNoAuth(requestConfig, appInfo.host.api,
				"1/oauth2/token_from_oauth1", null, getHeaders(token),
	...
	public void disableOAuth1AccessToken(DbxOAuth1AccessToken token)
		...
		if (token == null)
			throw new IllegalArgumentException("'token' can't be null");
		DbxRequestUtil.doPostNoAuth(requestConfig, appInfo.host.api,
				"1/disable_access_token", null, getHeaders(token),
	...
	private ArrayList<HttpRequestor.Header> getHeaders(
			DbxOAuth1AccessToken token) {
		ArrayList<HttpRequestor.Header> headers = new ArrayList<HttpRequestor.Header>(
				1);
		headers.add(new HttpRequestor.Header("Authorization",
				buildOAuth1Header(token)));
		return headers;
	}
	...
	private String buildOAuth1Header(DbxOAuth1AccessToken token) {
		...
		buf.append(", oauth_consumer_key=\"").append(encode(this.appInfo.key))
				.append("\"");
		buf.append(", oauth_token=\"").append(encode(token.key)).append("\"");
		buf.append(", oauth_signature=\"").append(encode(this.appInfo.secret))
				.append("&").append(encode(token.secret)).append("\"");
		return buf.toString();
	}
			...
			while (parser.getCurrentToken() == JsonToken.FIELD_NAME) {
				...
				try {
					if (fieldName.equals("token_type")) {
						tokenType = DbxAuthFinish.BearerTokenTypeReader
								.readField(parser, fieldName, tokenType);
					} else if (fieldName.equals("access_token")) {
						accessToken = DbxAuthFinish.AccessTokenReader
								.readField(parser, fieldName, accessToken);
					} else {
