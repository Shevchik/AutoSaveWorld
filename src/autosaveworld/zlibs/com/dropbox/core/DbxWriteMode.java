...
/**
 * Describes how a file should be saved when it is written to Dropbox. Do not
 * call the constructor. Instead, call one of the three static functions:
 * {@link #add}, {@link #force}, {@link #update}.
 */
public final class DbxWriteMode {
	// NOTE: The current implementation just holds the URL parameters that we
	// pass
	// to /files_put. If the upload mode becomes used in other calls, we may
	// need
	// to switch the implementation over to be less tied to that specific
	// endpoint.
	final String[] params;
	...
	private static final DbxWriteMode AddInstance = new DbxWriteMode(
			"overwrite", "false");

	/**
	 * Returns a {@link DbxWriteMode} for forcing a file to be at a certain
	 * path. If there's already a file at that path, the existing file will be
	 * overwritten. If there's a folder at that path, however, it will not be
	 * overwritten and the API call will fail.
	 */
	public static DbxWriteMode force() {
		return ForceInstance;
	}

	private static final DbxWriteMode ForceInstance = new DbxWriteMode(
			"overwrite", "true");

	/**
	 * Returns a {@link DbxWriteMode} for updating an existing file. This is
	 * useful for when you have downloaded a file and made modifications and
	 ...
	 * your contents to a new file. For example, if the original file path is
