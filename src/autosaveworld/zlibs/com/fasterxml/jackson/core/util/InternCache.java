...
/**
 * Singleton class that adds a simple first-level cache in front of regular
 * String.intern() functionality. This is done as a minor performance
 * optimization, to avoid calling native intern() method in cases where same
 * String is being interned multiple times.
 * <p>
 * Note: that this class extends {@link LinkedHashMap} is an implementation
 * detail -- no code should ever directly call Map methods.
 */
@SuppressWarnings("serial")
public final class InternCache extends ConcurrentHashMap<String, String> // since
																			// 2.3
{
	/**
	 ...
	 * {@link String#hashCode}; because of this, limit to reasonably low
	 * setting.
	 */
	...
	/**
	 * As minor optimization let's try to avoid "flush storms", cases where
	 * multiple threads might try to concurrently flush the map.
	 */
	...
	public String intern(String input) {
		...
		if (size() >= MAX_ENTRIES) {
			 ...
			 * underlying storage gives close enough answer to real one here;
			 * and we are more concerned with flooding than starvation.
			 */
			synchronized (lock) {
				if (size() >= MAX_ENTRIES) {
					clear();
