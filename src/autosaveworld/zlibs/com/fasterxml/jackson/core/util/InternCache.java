...
// 2.3
{
	...
	public String intern(String input) {
		...
		if (size() >= MAX_ENTRIES) {
			/*
			 * Not incorrect wrt well-known double-locking anti-pattern because underlying storage gives close enough answer to real one here; and we are more concerned with flooding than starvation.
			 */
			synchronized (lock) {
				if (size() >= MAX_ENTRIES) {
					clear();
