...
/**
 * Type abstraction that represents Java type that has been resolved (i.e. has
 * all generic information, if any, resolved to concrete types). Note that this
 * is an intermediate type, and all concrete instances MUST be of type
...
public abstract class ResolvedType {
	...
	/**
	 * Method for accessing key type for this type, assuming type has such a
	 * concept (only Map types do)
	 */
	public abstract ResolvedType getKeyType();

	/**
	 * Method for accessing content type of this type, if type has such a thing:
	 * simple types do not, structured types do (like arrays, Collections and
	 * Maps)
	 */
	public abstract ResolvedType getContentType();

	/**
	 * Method for checking how many contained types this type has. Contained
	 * types are usually generic types, so that generic Maps have 2 contained
	 * types.
	 */
	...
	/**
	 * Method that can be used to serialize type into form from which it can be
	 * fully deserialized from at a later point (using <code>TypeFactory</code>
	 * from mapper package). For simple types this is same as calling
