...
/**
 * Marker interface used to denote JSON Tree nodes, as far as the core package
 * knows them (which is very little): mostly needed to allow {@link ObjectCodec}
 * to have some level of interoperability. Most functionality is within
 * <code>JsonNode</code> base class in <code>mapper</code> package.
 * <p>
 * Note that in Jackson 1.x <code>JsonNode</code> itself was part of core
 * package: Jackson 2.x refactored this since conceptually Tree Model is part of
 * mapper package, and so part visible to <code>core</code> package should be
 * minimized.
 * <p>
 * NOTE: starting with Jackson 2.2, there is more functionality available via
 * this class, and the intent is that this should form actual base for multiple
 * alternative tree representations; for example, immutable trees could use
...
public interface TreeNode {
	...
	/**
	 * Method that can be used for efficient type detection when using stream
	 * abstraction for traversing nodes. Will return the first {@link JsonToken}
	 * that equivalent stream event would produce (for most nodes there is just
	...
	/**
	 * Method that returns true for all value nodes: ones that are not
	 * containers, and that do not represent "missing" nodes in the path. Such
	 * value nodes represent String, Number, Boolean and null values from JSON.
	 * <p>
	...
	/**
	 * Method that returns true for "virtual" nodes which represent missing
	 * entries constructed by path accessor methods when there is no actual node
	...
	/*
	 * /********************************************************** /* Basic
	 * traversal through structured entries (Arrays, Objects)
	 * /**********************************************************
	 */

	/**
	 * Method for accessing value of the specified field of an object node. If
	 * this node is not an object (or it does not have a value for specified
	 * field name), or if there is no field with such name, null is returned.
	 * <p>
	...
	/**
	 * Method for accessing value of the specified element of an array node. For
	 * other nodes, null is returned.
	 * <p>
	 * For array nodes, index specifies exact location within array and allows
	 * for efficient iteration over child elements (underlying storage is
	 * guaranteed to be efficiently indexable, i.e. has random-access to
	...
	/*
	 * /********************************************************** /* Converting
	 * to/from Streaming API
	 * /**********************************************************
	 */

	/**
	 * Method for constructing a {@link JsonParser} instance for iterating over
	 * contents of the tree that this node is root of. Functionally equivalent
	 * to first serializing tree using {@link ObjectCodec} and then re-parsing
	 * but more efficient.
	 */
	JsonParser traverse();

	/**
	 * Same as {@link #traverse()}, but additionally passes
