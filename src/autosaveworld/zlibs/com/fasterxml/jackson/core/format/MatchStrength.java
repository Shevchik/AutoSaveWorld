...
public enum MatchStrength {
	 ...
	 * could indicate a JSON String, which plausibly <b>could</b> indicate non-standard JSON usage).
	 */
	WEAK_MATCH,
	 ...
	 * <p>
	 * For example, when testing for XML data format, seeing a less-than character ("&lt;") alone (with possible leading spaces) would be a strong indication that data could be in xml format (but see below for {@link #FULL_MATCH} description for more)
	 */
	SOLID_MATCH,
	 ...
	 * <p>
	 * For example, when testing for XML data format, seing "&lt;xml" as the first data bytes ("XML declaration", as per XML specification) could give full confidence that data is indeed in XML format. Not all data formats have unique leading identifiers to allow full matches; for example, JSON only has heuristic matches and can have at most {@link #SOLID_MATCH}) match.
	 */
	FULL_MATCH;
}
