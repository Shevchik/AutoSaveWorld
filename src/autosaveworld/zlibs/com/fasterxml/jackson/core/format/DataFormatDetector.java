 ...
 * given an ordered set of {@link JsonFactory} instances to use for actual
 * low-level detection.
 */
public class DataFormatDetector {
	/**
	 * By default we will look ahead at most 64 bytes; in most cases, much less
	 * (4 bytes or so) is needed, but we will allow bit more leniency to support
	 * data formats that need more complex heuristics.
	 */
	public final static int DEFAULT_MAX_INPUT_LOOKAHEAD = 64;

	/**
	 * Ordered list of factories which both represent data formats to detect (in
	 * precedence order, starting with highest) and are used for actual
	 * detection.
	 */
	protected final JsonFactory[] _detectors;

	/**
	 * Strength of match we consider to be good enough to be used without
	 * checking any other formats. Default value is
	 * {@link MatchStrength#SOLID_MATCH},
	 */
	protected final MatchStrength _optimalMatch;

	/**
	 * Strength of minimal match we accept as the answer, unless better matches
	 * are found. Default value is {@link MatchStrength#WEAK_MATCH},
	 */
	protected final MatchStrength _minimalMatch;

	/**
	 * Maximum number of leading bytes of the input that we can read to
	...
	/**
	 * Method that will return a detector instance that uses given optimal match
	 * level (match that is considered sufficient to return, without trying to
	 * find stronger matches with other formats).
	 */
	public DataFormatDetector withOptimalMatch(MatchStrength optMatch) {
		if (optMatch == _optimalMatch) {
			return this;
		}
		return new DataFormatDetector(_detectors, optMatch, _minimalMatch,
				_maxInputLookahead);
	}

	/**
	 * Method that will return a detector instance that uses given minimal match
	 * level; match that may be returned unless a stronger match is found with
	...
	public DataFormatDetector withMinimalMatch(MatchStrength minMatch) {
		...
		return new DataFormatDetector(_detectors, _optimalMatch, minMatch,
				_maxInputLookahead);
	}

	/**
	 * Method that will return a detector instance that allows detectors to read
	 * up to specified number of bytes when determining format match strength.
	 */
	public DataFormatDetector withMaxInputLookahead(int lookaheadBytes) {
		if (lookaheadBytes == _maxInputLookahead) {
			return this;
		}
		return new DataFormatDetector(_detectors, _optimalMatch, _minimalMatch,
				lookaheadBytes);
	}

	private DataFormatDetector(JsonFactory[] detectors, MatchStrength optMatch,
			MatchStrength minMatch, int maxInputLookahead) {
	...
	public DataFormatMatcher findFormat(InputStream in) throws IOException {
		return _findFormat(new InputAccessor.Std(in,
				new byte[_maxInputLookahead]));
	}
	...
	private DataFormatMatcher _findFormat(InputAccessor.Std acc)
		...
		for (JsonFactory f : _detectors) {
			...
			// limit), skip
			if (strength == null
					|| strength.ordinal() < _minimalMatch.ordinal()) {
				continue;
			}
