...
/**
 * This class is a kind of specialized type-safe Map, from char array to String
 * value. Specialization means that in addition to type-safety and specific
 * access patterns (key char array, Value optionally interned String; values
 * added on access if necessary), and that instances are meant to be used
 * concurrently, but by using well-defined mechanisms to obtain such
 * concurrently usable instances. Main use for the class is to store symbol
 * table information for things like compilers and parsers; especially when
 * number of symbols (keywords) is limited.
 * <p>
 * For optimal performance, usage pattern should be one where matches should be
 * very common (especially after "warm-up"), and as with most hash-based
 * maps/sets, that hash codes are uniformly distributed. Also, collisions are
 * slightly more expensive than with HashMap or HashSet, since hash codes are
 * not used in resolving collisions; that is, equals() comparison is done with
 * all symbols in same bucket index.<br />
 * Finally, rehashing is also more expensive, as hash codes are not stored;
 * rehashing requires all entries' hash codes to be recalculated. Reason for not
 * storing hash codes is reduced memory usage, hoping for better memory
 * locality.
 * <p>
 * Usual usage pattern is to create a single "master" instance, and either use
 * that instance in sequential fashion, or to create derived "child" instances,
 * which after use, are asked to return possible symbol additions to master
 * instance. In either case benefit is that symbol table gets initialized so
 * that further uses are more efficient, as eventually all symbols needed will
 * already be in symbol table. At that point no more Symbol String allocations
 * are needed, nor changes to symbol table itself.
 * <p>
 * Note that while individual SymbolTable instances are NOT thread-safe (much
 * like generic collection classes), concurrently used "child" instances can be
 * freely used without synchronization. However, using master table concurrently
 * with child instances can only be done if access to master instance is
 * read-only (i.e. no modifications done).
 */
public final class CharsToNameCanonicalizer {
	/*
	 * If we use "multiply-add" based hash algorithm, this is the multiplier we
	 * use.<p> Note that JDK uses 31; but it seems that 33 produces fewer
	 * collisions, at least with tests we have.
	 */
	public final static int HASH_MULT = 33;

	/**
	 * Default initial table size. Shouldn't be miniscule (as there's cost to
	 * both array realloc and rehashing), but let's keep it reasonably small.
	 * For systems that properly reuse factories it doesn't matter either way;
	 * but when recreating factories often, initial overhead may dominate.
	 */
	 ...
	 * Let's start with static value of 255 entries for the longest legal chain.
	 * <p>
	 * Note: longest chain we have been able to produce without malicious intent
	 * has been 38 (with
	 * "com.fasterxml.jackson.core.main.TestWithTonsaSymbols"); our setting
	...
	/**
	 * Sharing of learnt symbols is done by optional linking of symbol table
	 * instances with their parents. When parent linkage is defined, and child
	 * instance is released (call to <code>release</code>), parent's shared
	 * tables may be updated from the child instance.
	 */
	...
	/**
	 * Whether any canonicalization should be attempted (whether using intern or
	 * not)
	 */
	...
	/**
	 * Limit that indicates maximum size this instance can hold before it needs
	 * to be expanded and rehashed. Calculated using fill factor passed in to
	 * constructor.
	 */
	 ...
	 * <code>_buckets.length - 1</code>, when _buckets.length is a power of two.
	 */
	...
	/*
	 * /********************************************************** /* State
	 * regarding shared arrays
	 * /**********************************************************
	 */

	/**
	 * Flag that indicates if any changes have been made to the data; used to
	 * both determine if bucket array needs to be copied when (first) change is
	 * made, and potentially if updated bucket list is to be resync'ed back to
	 * master instance.
	 */
	 ...
	 * {@link autosaveworld.zlibs.com.fasterxml.jackson.core.JsonFactory} instance. Root instance is
	 * never used directly; its main use is for storing and sharing underlying
	 * symbol arrays as needed.
	 */
	...
	private CharsToNameCanonicalizer(CharsToNameCanonicalizer parent,
			int flags, String[] symbols, Bucket[] buckets, int size,
			int hashSeed, int longestColl) {
		_parent = parent;

		_flags = flags;
		_canonicalize = JsonFactory.Feature.CANONICALIZE_FIELD_NAMES
				.enabledIn(flags);
	 ...
	 * It will be a copy-on-write instance, ie. it will only use read-only copy
	 * of parent's data, but when changes are needed, a copy will be created.
	 * <p>
	 * Note: while this method is synchronized, it is generally not safe to both
	 * use makeChild/mergeChild, AND to use instance actively. Instead, a
	 * separate 'root' instance should be used on which only
	 * makeChild/mergeChild are called, but instance itself is not used as a
	 * symbol table.
	 */
	public CharsToNameCanonicalizer makeChild(int flags) {
		/*
		 * 24-Jul-2012, tatu: Trying to reduce scope of synchronization,
		 * assuming that synchronizing construction is the (potentially)
		 * expensive part, and not so much short copy-the-variables thing.
		 */
		...
		return new CharsToNameCanonicalizer(this, flags, symbols, buckets,
				size, hashSeed, longestCollisionList);
	}

	private CharsToNameCanonicalizer makeOrphan(int seed) {
		return new CharsToNameCanonicalizer(null, -1, _symbols, _buckets,
				_size, seed, _longestCollisionList);
	}

	/**
	 * Method that allows contents of child table to potentially be "merged in"
	 * with contents of this symbol table.
	 * <p>
	 * Note that caller has to make sure symbol table passed in is really a
	 * child or sibling of this symbol table.
	 */
	...
	public String findSymbol(char[] buffer, int start, int len, int h) {
		...
		/*
		 * Related to problems with sub-standard hashing (somewhat relevant for
		 * collision attacks too), let's try little bit of shuffling to improve
		 * hash codes. (note, however, that this can't help with full
		 * collisions)
		 */
	...
	private String _addSymbol(char[] buffer, int start, int len, int h,
			int index) {
		...
		if (_symbols[index] == null) {
			...
			if (collLen > MAX_COLL_CHAIN_LENGTH) {
				...
				_buckets[bix] = newB;
				_longestCollisionList = Math
						.max(collLen, _longestCollisionList);
			}
		}
	...
	private void _handleSpillOverflow(int bindex, Bucket newBucket) {
		if (_overflows == null) {
			...
			if (_overflows.get(bindex)) {
				...
				if (JsonFactory.Feature.FAIL_ON_SYMBOL_HASH_OVERFLOW
						.enabledIn(_flags)) {
	...
	/**
	 * Implementation of a hashing method for variable length Strings. Most of
	...
	/**
	 * Method called when copy-on-write is needed; generally when first change
	 * is made to a derived symbol table.
	 */
	...
	/**
	 * Method called when size (number of entries) of symbol table grows so big
	 * that load factor is exceeded. Since size has to remain power of two,
	 * arrays will then always be doubled. Main work is really redistributing
	 * old entries into new String/Bucket entries.
	 */
	private void rehash() {
		...
		/*
		 * 12-Mar-2010, tatu: Let's actually limit maximum size we are prepared
		 * to use, to guard against OOME in case of unbounded name sets (unique
		 * [non-repeating] names)
		 */
		if (newSize > MAX_T_SIZE) {
			/*
			 * If this happens, there's no point in either growing or shrinking
			 * hash areas. Rather, let's just cut our losses and stop
			 * canonicalizing.
			 */
			_size = 0;
			_canonicalize = false;
		...
		/*
		 * Need to do two loops, unfortunately, since spill-over area is only
		 * half the size:
		 */
		int maxColl = 0;
		...
		if (count != _size) {
			throw new Error("Internal error on SymbolTable.rehash(): had "
					+ _size + " entries; now have " + count + ".");
		}
	}
