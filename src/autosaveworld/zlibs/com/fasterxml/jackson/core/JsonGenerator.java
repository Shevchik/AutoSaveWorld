...
/**
 * Base class that defines public API for writing JSON content. Instances are created using factory methods of a {@link JsonFactory} instance.
 *
...
public abstract class JsonGenerator implements Closeable, Flushable, Versioned {
	...
	/**
	 * Method that can be called to set or reset the object to use for writing Java objects as JsonContent (using method {@link #writeObject}).
	 *
	 ...
	 * <p>
	 * To use the default pretty printer that comes with core Jackson distribution, call {@link #useDefaultPrettyPrinter} instead.
	 *
	 * @return Generator itself (this), to allow chaining
	 */
	...
	/**
	 * Accessor for checking whether this generator has a configured {@link PrettyPrinter}; returns it if so, null if none configured.
	 *
	 * @since 2.1
	 */
	...
	/**
	 * Convenience method for enabling pretty-printing using the default pretty printer ({@link autosaveworld.zlibs.com.fasterxml.jackson.core.util.DefaultPrettyPrinter}).
	 *
	 * @return Generator itself (this), to allow chaining
	 */
	 ...
	 *
	 * @param charCode
	 *            Either -1 to indicate that no additional escaping is to be done; or highest code point not to escape (meaning higher ones will be), if positive value.
	 */
	...
	/**
	 * Method that can be used to verify that given schema can be used with this generator (using {@link #setSchema}).
	 *
	 * @param schema
	 *            Schema to check
	 *
	 ...
	 *
	 * @param size
	 *            Number of elements this array will have: actual number of values written (before matching call to {@link #writeEndArray()} MUST match; generator MAY verify this is the case.
	 */
	...
	/**
	 * Similar to {@link #writeBinary(Base64Variant,byte[],int,int)}, but default to using the Jackson default Base64 variant (which is {@link Base64Variants#MIME_NO_LINEFEEDS}).
	 */
	...
	/**
	 * Similar to {@link #writeBinary(Base64Variant,InputStream,int)}, but assumes default to using the Jackson default Base64 variant (which is {@link Base64Variants#MIME_NO_LINEFEEDS}).
	 *
	 * @param data
	...
	/**
	 * Method similar to {@link #writeBinary(Base64Variant,byte[],int,int)}, but where input is provided through a stream, allowing for incremental writes without holding the whole input in memory.
	 *
	 * @param bv
	...
	/**
	 * Method called to indicate that a property in this position was skipped. It is usually only called for generators that return <code>false</code> from {@link #canOmitFields()}.
	 * <p>
	...
	/**
	 * Method for copying contents of the current event <b>and following events that it encloses</b> the given parser instance points to.
	 * <p>
	...
	/**
	 * Helper method used for constructing and throwing {@link JsonGenerationException} with given base message.
	 * <p>
	 * Note that sub-classes may override this method to add more detail or use a {@link JsonGenerationException} sub-class.
	 */
	...
	protected void _writeSimpleObject(Object value) throws IOException {
		/*
		 * 31-Dec-2009, tatu: Actually, we could just handle some basic types even without codec. This can improve interoperability, and specifically help with TokenBuffer.
		 */
		if (value == null) {
			writeNull();
			return;
