...
/**
 * Enumeration for basic token types used for returning results of parsing JSON
 * content.
 */
public enum JsonToken {
	...
	/**
	 * NOT_AVAILABLE can be returned if {@link JsonParser} implementation can
	 * not currently return the requested token (usually next one), or even if
	 * any will be available, but that may be able to determine this in future.
	 * This is the case with non-blocking parsers -- they can not block to wait
	 * for more data to parse and must return something.
	 */
	NOT_AVAILABLE(null, JsonTokenId.ID_NOT_AVAILABLE),

	/**
	 * START_OBJECT is returned when encountering '{' which signals starting of
	 * an Object value.
	 */
	START_OBJECT("{", JsonTokenId.ID_START_OBJECT),

	/**
	 * END_OBJECT is returned when encountering '}' which signals ending of an
	 * Object value
	 */
	END_OBJECT("}", JsonTokenId.ID_END_OBJECT),

	/**
	 * START_ARRAY is returned when encountering '[' which signals starting of
	 * an Array value
	 */
	START_ARRAY("[", JsonTokenId.ID_START_ARRAY),

	/**
	 * END_ARRAY is returned when encountering ']' which signals ending of an
	 * Array value
	 */
	...
	/**
	 * Placeholder token returned when the input source has a concept of
	 * embedded Object that are not accessible as usual structure (of starting
	 * with {@link #START_OBJECT}, having values, ending with
	 * {@link #END_OBJECT}), but as "raw" objects.
	 * <p>
	 * Note: this token is never returned by regular JSON readers, but only by
	 * readers that expose other kinds of source (like <code>JsonNode</code>
	 ...
	 * context (array element, field value, or root-level stand-alone value)
	 */
	VALUE_STRING(null, JsonTokenId.ID_STRING),

	/**
	 * VALUE_NUMBER_INT is returned when an integer numeric token is encountered
	 * in value context: that is, a number that does not have floating point or
	 * exponent marker in it (consists only of an optional sign, followed by one
	 * or more digits)
	 */
	VALUE_NUMBER_INT(null, JsonTokenId.ID_NUMBER_INT),

	/**
	 * VALUE_NUMBER_INT is returned when a numeric token other that is not an
	 * integer is encountered: that is, a number that does have floating point
	 * or exponent marker in it, in addition to one or more digits.
	 */
	...
	/**
	 * @param token
	 *            representation for this token, if there is a single static
	 *            representation; null otherwise
	 */
	 ...
	 * non-structured value. This means all tokens other than Object/Array
	 * start/end markers all field names.
	 */
