 ...
 * reader (aka parser, {@link JsonParser}) and writer (aka generator,
...
public class JsonFactory implements Versioned, java.io.Serializable {
	...
	public enum Feature {
		 ...
		 * -- which almost certainly means a denial-of-service attack via
		 * generated duplicate hash codes. If feature is enabled, an
		 * {@link IllegalStateException} is thrown to indicate the suspected
		 * denial-of-service attack; if disabled, processing continues but
		 * canonicalization (and thereby <code>intern()</code>ing) is disabled)
	 ...
	 * default.
	 */
	protected final static int DEFAULT_PARSER_FEATURE_FLAGS = JsonParser.Feature
			.collectDefaults();
	 ...
	 */
	protected final transient CharsToNameCanonicalizer _rootCharSymbols = CharsToNameCanonicalizer
			.createRoot();
	 ...
	 * <p>
	 * TODO: should clean up this; looks messy having 2 alternatives with not
	 * very clear differences.
	 */
	protected final transient BytesToNameCanonicalizer _rootByteSymbols = BytesToNameCanonicalizer
			.createRoot();
	...
	protected JsonFactory(JsonFactory src, ObjectCodec codec) {
		...
		/*
		 * 27-Apr-2013, tatu: How about symbol table; should we try to reuse
		 * shared symbol tables? Could be more efficient that way; although can
		 * slightly add to concurrency overhead.
		 */
	}

	/**
	 * Method for constructing a new {@link JsonFactory} that has the same
	 * settings as this instance, but is otherwise independent (i.e. nothing is
	 * actually shared, symbol tables are separate). Note that
	 * {@link ObjectCodec} reference is not copied but is set to null; caller
	 * typically needs to set it after calling this method. Reason for this is
	...
	protected void _checkInvalidCopy(Class<?> exp) {
		if (getClass() != exp) {
			throw new IllegalStateException("Failed copy(): "
					+ getClass().getName() + " (version: " + version()
	...
	/**
	 * Introspection method that higher-level functionality may call to see
	 * whether underlying data format requires a stable ordering of object
	 * properties or not. This is usually used for determining whether to force
	 * a stable ordering (like alphabetic ordering by name) if no ordering if
	...
	/**
	 * Introspection method that higher-level functionality may call to see
	 * whether underlying data format can read and write binary data natively;
	 * that is, embeded it as-is without using encodings such as Base64.
	 * <p>
	 * Default implementation returns <code>false</code> as JSON does not
	...
	/**
	 * Introspection method that can be used by base factory to check whether
	 * access using <code>char[]</code> is something that actual parser
	 * implementations can take advantage of, over having to use
	 * {@link java.io.Reader}. Sub-types are expected to override definition;
	...
	/**
	 * Method that can be used to quickly check whether given schema is
	 * something that parsers and/or generators constructed by this factory
	 * could use. Note that this means possible use, at the level of data format
	...
	/**
	 * Helper method that can be called to determine if content accessed using
	 * given accessor seems to be JSON content.
	 */
	 ...
	 * {@link JsonParser.Feature} for list of features)
	 */
	...
	public String getRootValueSeparator() {
		return (_rootValueSeparator == null) ? null : _rootValueSeparator
				.getValue();
	}
	 ...
	 * This is needed to use data-binding methods of {@link JsonParser} and
	 * {@link JsonGenerator} instances.
	 */
	...
	/*
	 * /********************************************************** /* Parser
	 * factories (new ones, as per [Issue-25])
	 * /**********************************************************
	 */
	...
	/*
	 * /********************************************************** /* Parser
	 * factories (old ones, as per [Issue-25])
	 * /**********************************************************
	 */
	...
	protected JsonParser _createParser(Reader r, IOContext ctxt)
			throws IOException {
		return new ReaderBasedJsonParser(ctxt, _parserFeatures, r,
				_objectCodec, _rootCharSymbols.makeChild(_factoryFeatures));
	}
	...
	protected JsonParser _createParser(char[] data, int offset, int len,
		...
		return new ReaderBasedJsonParser(ctxt, _parserFeatures, null,
				_objectCodec, _rootCharSymbols.makeChild(_factoryFeatures),
	...
	protected JsonParser _createParser(byte[] data, int offset, int len,
		...
		return new ByteSourceJsonBootstrapper(ctxt, data, offset, len)
				.constructParser(_parserFeatures, _objectCodec,
						_rootByteSymbols, _rootCharSymbols, _factoryFeatures);
	}
	...
	/*
	 * /********************************************************** /* Internal
	 * factory methods, decorator handling
	 * /**********************************************************
	 */
	...
	protected InputStream _optimizedStreamFromURL(URL url) throws IOException {
		if ("file".equals(url.getProtocol())) {
			...
			if (host == null || host.length() == 0) {
				...
				// otherwise, let's fall through and let URL decoder do its
				// magic
			}
		}
